...
class DynBind(object):
    ...
    def __getattribute__(self, name):
        ...
        suffix = object.__getattribute__(self, 'suffix')
        return DynBindSlave(name, suffix)
...
def _reload_modules(cls, reloading=True):
    ...
    for modname in cls._moddirs:
        ...
        clssubmodname = '.'.join([cls.__module__.lower(), modname.lower(), cls.__name__.lower()])
...
def _bindfuncs(self):
    ...
    for mname, method in methods:
        ...
        for dynbinding in dynbindings:
            if dynbinding:
                eventname, widgetprefix, widgetname = dynbinding[0:3]
                impmodchain = ['wx',]
                if len(dynbinding) > 3:
                    impmodchain.extend(dynbinding[3:])
                impmodname = '.'.join(impmodchain)
                print 'looking for event in', impmodname
                
                impmod = sys.modules[impmodname]
                event = getattr(impmod, eventname, None)
                if event is None:
                    print 'Method', mname
...
DynamicViewController = functools.partial(DynamicClass(moddirs=['views', 'controllers']))
DynamicModel = functools.partial(DynamicClass(moddirs=['models',]))
